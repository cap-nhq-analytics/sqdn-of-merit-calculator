#' @title Calculate the Achievement Metric
#' @description Calculate the achievement metric for all units in a Squadron of Merit Listing.
#' @details
#' By default, the achievement metric awards 1 point for every Wright Brothers Award, 2 points for
...
#' @return A data.table containing the unit charter number and the achievement metric.
#' @export

calculate_achv_metric <- function(som_report, award_weights = c(1, 2, 2, 3, 3)) {
    ...
    result[j = achv_metric := scale(raw_score) + 1]
    result <- result[j = list(RWU, achv_metric)]
    result[]
    return(result)
}
